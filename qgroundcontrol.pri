...
win32-msvc2008|win32-msvc2010 {
	...
	CONFIG(debug, debug|release) {
		...
		QMAKE_POST_LINK += $$quote(xcopy /D /Y "$$(QTDIR)\\bin\\d4.dll" "$$TARGETDIR_WIN\\debug"$$escape_expand(\\n))
	...
	CONFIG(release, debug|release) {
		...
		QMAKE_POST_LINK += $$quote(xcopy /D /Y "$$(QTDIR)\\bin\\4.dll" "$$TARGETDIR_WIN\\release"$$escape_expand(\\n))
